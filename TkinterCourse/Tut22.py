...
def friend():
    ans=tmsg.askyesno("Be My Friend?", "You Be My Friend")
    if ans:
        msg="You are my friend now"
    else:
        msg="You are my enemy now"
    tmsg.showinfo("Be My Friend?", msg)

def warn():
    tmsg.showwarning("Warning", "This is warning")
...
m3.add_command(label="AskBeMyFriend", command=friend)
m3.add_command(label="Warning", command=warn)
